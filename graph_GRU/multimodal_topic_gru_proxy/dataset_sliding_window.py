...
from .._multimodal_model_gru.GAT_sliding_window import GATJKClassifier
...
class MultimodalData(Data):
  def __inc__(self, key, value, *args, **kwargs):
    # vision_node_map은 해당 그래프의 vision 노드 개수만큼 증가시켜야 함
    if key == 'vision_node_map':
      return self.num_vision_nodes
    # audio_node_map은 해당 그래프의 audio 노드 개수만큼 증가시켜야 함
    if key == 'audio_node_map':
      return self.num_audio_nodes
    # 나머지는 기본 동작 (edge_index 등은 num_nodes 기반 증가)
...
def make_graph(
  ...
  :param max_sequence: Max sequence for GRU
    ...
    for graph_idx, id in tqdm(enumerate(filtered_ids), desc="Dataframe -> Graph", total=len(filtered_ids)):
      ...
      try:
        ...
        for t_emb, topic, (start, stop) in zip(t_embeds, topics, start_stop_list):
          ...
          if len(v_target) > 0:
            num_frames = len(v_target)
            v_windows = []

            for i in range(0, max(1, num_frames - stride + 1), stride):
              segment = v_target[i : i + window_size]
              actual_len = len(segment)
            
              if actual_len < window_size:
                win = pad_sequence_numpy(segment, window_size)
              else:
                win = segment
              
              v_windows.append(win)
              vision_lengths_list.append(min(actual_len, window_size))

            if not v_windows:
              v_windows.append(pad_sequence_numpy(v_target, window_size))
              logger.warning("No windows detected for Vision")
              vision_lengths_list.append(min(num_frames, window_size))

            vision_seq_list.append(v_windows)
          ...
          if len(a_target) > 0:
            ...
            if a_tensor.size(-1) >= down_k:
              downsampled_a_tensor = F.avg_pool1d(a_tensor, kernel_size=down_k)
            else:
              downsampled_a_tensor = a_tensor

            processed_audio = downsampled_a_tensor.squeeze(0).T.numpy()

            num_frames = len(processed_audio)
            a_windows = []

            for i in range(0, max(1, num_frames - stride + 1), stride):
              segment = processed_audio[i : i + window_size]
              actual_len = len(segment)
              ...
              a_windows.append(win)
              audio_lengths_list.append(min(actual_len, window_size))

            if not a_windows:
              a_windows.append(pad_sequence_numpy(processed_audio, window_size))
              logger.warning("No windows detected for Audio")
              audio_lengths_list.append(min(num_frames, window_size))

            audio_seq_list.append(a_windows)
        ...
        for node_idx, windows in enumerate(vision_seq_list):
          for win in windows:
            vision_windows.append(win)
            vision_node_map.append(node_idx)

        # Audio
        audio_windows = []
        audio_node_map = []

        for node_idx, windows in enumerate(audio_seq_list):
          for win in windows:
            audio_windows.append(win)
            audio_node_map.append(node_idx)

        edge_index = torch.tensor([source_nodes, target_nodes], dtype=torch.long)
        y = torch.tensor([filtered_labels[graph_idx]], dtype=torch.long)

        data = MultimodalData(x=x, edge_index=edge_index, y=y, node_types=node_types)

        data.x_vision = torch.tensor(np.array(vision_windows), dtype=torch.float)
        data.x_audio = torch.tensor(np.array(audio_windows), dtype=torch.float)
        data.vision_node_map = torch.tensor(vision_node_map, dtype=torch.long)
        data.audio_node_map = torch.tensor(audio_node_map, dtype=torch.long)
        data.vision_lengths = torch.tensor(vision_lengths_list, dtype=torch.long)
        data.audio_lengths = torch.tensor(audio_lengths_list, dtype=torch.long)

        data.num_vision_nodes = len(vision_seq_list)
        data.num_audio_nodes = len(audio_seq_list)

        if visualization:
          logger.info(f"X_Vision: {data.x_vision.shape}")
          logger.info(f"X_Audio: {data.x_audio.shape}")
          logger.info(f"X_Vision_Map: {data.vision_node_map.shape}")
          logger.info(f"X_Audio_Map: {data.audio_node_map.shape}")
...
if __name__=="__main__":
  ...
  plt.axis('off')
  plt.tight_layout()
  save_path = os.path.join(save_dir, f'{os.path.basename(os.path.dirname(os.path.abspath(__file__)))}_sliding_windows_graph_img.png')
