...
from ..train_val import train_gat, validation_gat, check_gru_grad, FocalLoss
import pandas as pd
...
def main():
  ...
  parser.add_argument('--config', type=str, default='graph_GRU/configs/best_config.yaml',
  ...
  # --- 수정 시작: 전처리된 데이터 로드 ---
  preprocessed_dir = os.path.join(path_config.ROOT_DIR, "preprocessed_data")
  train_pt_path = os.path.join(preprocessed_dir, 'train_graphs.pt')
  val_pt_path = os.path.join(preprocessed_dir, 'val_graphs.pt')

  if os.path.exists(train_pt_path) and os.path.exists(val_pt_path):
    logger.info("Loading preprocessed graphs from disk...")
    # 전처리 스크립트에서 저장한 형식대로 불러오기
    train_data = torch.load(train_pt_path, weights_only=False)
    val_data = torch.load(val_pt_path, weights_only=False)

    train_graphs = train_data['graphs']
    t_dim, v_dim, a_dim = train_data['t_dim'], train_data['v_dim'], train_data['a_dim']
    val_graphs = val_data['graphs']
    
    logger.info(f"Loaded {len(train_graphs)} train graphs and {len(val_graphs)} val graphs.")
  else:
    # 만약 파일이 없으면 기존처럼 생성 (보험용 로직)
    logger.warning("Preprocessed files not found! Processing from scratch...")
    gc = TopicProxyGRU_GC()
    train_graphs, (t_dim, v_dim, a_dim) = gc.make_graph(
      ids = train_id + val_id,
      labels = train_label + val_label,
...
# first
#   ex) python -m graph_GRU.multimodal_topic_gru_proxy.train --save_dir checkpoints_graph5 --save_dir_ multimodal_topic_gru_proxy_v2 --num_epochs 300 --patience 20 --version 2
# resume
