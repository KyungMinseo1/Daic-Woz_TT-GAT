...
from .._bimodal_model_gru.GAT import GATClassifier, GATJKClassifier
from ..graph_construct import GraphConfig
from .dataset import BiTopicProxyGRU_GC
...
def main():
  ...
  parser.add_argument('--config', type=str, default='graph_GRU/configs/architecture_TT_GAT.yaml',
  ...
  parser.add_argument('--colab_path', type=str, default=None, 
                      help="Write the path of temporary directory when using colab. (Transcription/Vision)")
  ...
  gc = BiTopicProxyGRU_GC()

  train_graphs, (t_dim, v_dim) = gc.make_graph(
    ids = train_id+val_id,
    labels = train_label+val_label,
    config = graph_config
  )
  logger.info("Processing Validation Data")
  val_graphs, (_, _) = gc.make_graph(
    ids = test_id,
    labels = test_label,
    config = graph_config
  )
