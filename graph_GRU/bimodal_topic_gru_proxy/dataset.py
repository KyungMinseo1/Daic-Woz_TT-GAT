...
class BiTopicProxyGRU_GC(Graph_Constructor):
  def make_graph(
      self,
      ids : list,
      labels : list,
    ...
    try:
      ...
      for graph_idx, id_ in tqdm(enumerate(filtered_ids), desc="Dataframe -> Graph", total=len(filtered_ids)):
        try:
          vision_df, _, utterances, topics, start_stop_list, start_stop_list_ellie = \
            self.prepare_df(
              id_ = id_,
      ...
      if config.explanation:
        return graphs, (text_dim, v_dim), (topic_node_id_dict, utterances, vision_seq_list)
      else:
        return graphs, (text_dim, v_dim)
