...
class AttentionGRU(nn.Module):
    def __init__(self, input_dim:int, hidden_dim:int, output_dim:int, num_layers:int=2, dropout:float=0.3):
        super().__init__()
        self.gru = nn.GRU(input_dim, hidden_dim, num_layers=num_layers, batch_first=True, bidirectional=False, dropout=dropout if num_layers>1 else 0.0)
...
class GATJKClassifier(nn.Module):
    def __init__(
        ...
        if self.use_attention and hasattr(self.vision_gru, 'gru'):
            self.vision_gru.gru.flatten_parameters()
            self.audio_gru.gru.flatten_parameters()
        elif hasattr(self.vision_gru, 'gru'):
            self.vision_gru.gru.flatten_parameters()
            self.audio_gru.gru.flatten_parameters()
    ...
    def forward(self, data, explanation=False):
        ...
        if x_vision.size(0) > 0 and len(vision_indices) > 0:
            h_win_v, _ = self.vision_gru(x_vision, vision_lengths) # (Total_V_Windows, Hidden)
        ...
        if x_audio.size(0) > 0 and len(audio_indices) > 0:
            h_win_a, _ = self.audio_gru(x_audio, audio_lengths) # (Total_A_Windows, Hidden)
