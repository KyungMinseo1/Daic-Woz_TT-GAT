...
class NCEMultimodalModel(nn.Module):
  ...
  def forward(self, x, x_len, x2, x2_len, x_queue, x2_queue):
    ...
    Args:
      ...
      x_queue: (D, K) - x queue features
      x2_queue: (D, K) - x2 queue features
    """
    B = x.size(0) # Extract batch size
    K = x_queue.size(0) # Extract queue size

    q = F.normalize(self.encoder_x_q(x, x_len), dim=1)  # (B, C) -> transcription LSTM feat
    q2 = F.normalize(self.encoder_x2_q(x2, x2_len), dim=1)  # (B, C) -> vision or audio LSTM feat

    C = q.size(1)

    with torch.no_grad():
      k = F.normalize(self.encoder_x_k(x, x_len), dim=1)  # (B, C) -> transcription LSTM feat
      k2 = F.normalize(self.encoder_x2_k(x2, x2_len), dim=1)  # (B, C) -> vision or audio LSTM feat

    # q: transcription, k: vision
    # Positive logits
    l1_pos = torch.bmm(q.view(B, 1, C), k2.view(B, C, 1)) # (B, 1)
    # Negative logits
    l1_neg = torch.mm(q, x2_queue.view(C, K)) # (B, K)
    # logits
    logits1 = torch.cat([l1_pos, l1_neg], dim=1) # (B, 1+K)
    # contrastive loss
    labels1 = torch.zeros(B)

    # q: vision, k: transcription
    # Positive logits
    l2_pos = torch.bmm(q2.view(B, 1, C), k.view(B, C, 1)) # (B, 1)
    # Negative logits
    l2_neg = torch.mm(q2, x_queue.view(C, K)) # (B, K)
    # logits
    logits2 = torch.cat([l2_pos, l2_neg], dim=1) # (B, 1+K)
    # contrastive loss
    labels2 = torch.zeros(B)
