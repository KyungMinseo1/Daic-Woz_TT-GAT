...
from dataset_unimodal import make_graph
...
def train_gat(train_loader, model, criterion, optimizer, device, num_classes=2):  
  ...
  for idx, batch in enumerate(pbar):
    ...
    if num_classes == 2:
      ...
      pred = (torch.sigmoid(out) > 0.4).long()
    else:
...
def main():
  ...
  train_graphs, _ = make_graph(train_id, train_label, model_name=config['training']['embed_model'])
  logger.info("Processing Validation Data")
  val_graphs, _ = make_graph(val_id, val_label, model_name=config['training']['embed_model'])
