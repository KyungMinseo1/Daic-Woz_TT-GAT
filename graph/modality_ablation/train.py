...
from .GAT import GATClassifier, GATJKClassifier
...
def main():
  ...
  train_graphs, (t_dim, v_dim, a_dim) = make_graph(
    ...
    use_vision=config['model']['use_vision'],
  ...
  logger.info("="*40)
  logger.info("       MODALITY CONFIGURATION")
  logger.info("="*40)
  logger.info(f"  > Text Modality:   {'[ON]' if config['model'].get('use_text', True) else '[OFF]'}")
  logger.info(f"  > Vision Modality: {'[ON]' if config['model'].get('use_vision', True) else '[OFF]'}")
  logger.info(f"  > Audio Modality:  {'[ON]' if config['model'].get('use_audio', True) else '[OFF]'}")
  logger.info(f"  > Summary Node:    {'[ON]' if config['model'].get('use_summary_node', True) else '[OFF]'}")
  logger.info("="*40)
