...
class GATClassifier(nn.Module):
    def __init__(
        ...
        # Vision LSTM
        if self.use_vision and vision_dim > 0:
            LSTMClass = AttentionBiLSTM if use_attention else SimpleBiLSTM
            self.vision_lstm = LSTMClass(
        ...
        # Audio LSTM
        if self.use_audio and audio_dim > 0:
            LSTMClass = AttentionBiLSTM if use_attention else SimpleBiLSTM
            self.audio_lstm = LSTMClass(
                input_dim=audio_dim,
                hidden_dim=hidden_channels,
                output_dim=hidden_channels,
                num_layers=bilstm_num_layers,
        ...
        if self.audio_lstm is not None:
    ...
    def forward(self, data, explanation=False):
        ...
        x_vision = getattr(data, 'x_vision', None)
        x_audio = getattr(data, 'x_audio', None)
        vision_lengths = getattr(data, 'vision_lengths', None)
        audio_lengths = getattr(data, 'audio_lengths', None)
        ...
        # Vision
        if self.use_vision and self.vision_lstm is not None and x_vision.size(0) > 0:
            vision_indices = [i for i, t in enumerate(flat_node_types) if t == 'vision']
            if vision_indices:
                h_vision, _ = self.vision_lstm(x_vision, vision_lengths)
                final_x[vision_indices] = h_vision.to(x.dtype)

        # Audio
        if self.use_audio and self.audio_lstm is not None and x_audio.size(0) > 0:
            audio_indices = [i for i, t in enumerate(flat_node_types) if t == 'audio']
            if audio_indices:
...
class GATJKClassifier(nn.Module):
    def __init__(
        ...
        # Vision LSTM
        if self.use_vision and vision_dim > 0:
            LSTMClass = AttentionBiLSTM if self.use_attention else SimpleBiLSTM
            self.vision_lstm = LSTMClass(
                input_dim=vision_dim, 
                hidden_dim=hidden_channels,
                output_dim=hidden_channels,
        ...
        if self.vision_lstm is not None: self.vision_lstm.bilstm.flatten_parameters()
        if self.audio_lstm is not None: self.audio_lstm.bilstm.flatten_parameters()
        self._init_weights()
