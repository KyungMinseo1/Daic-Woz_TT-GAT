...
def make_graph(ids, labels, model_name, colab_path=None,
               use_summary_node=True, t_t_connect=False, v_a_connect=False,
               visualization=False, explanation=False,
               use_vision=True, use_text=True, use_audio=True,):
  try:
    ...
    for graph_idx, id in tqdm(enumerate(filtered_ids), desc="Dataframe -> Graph", total=len(filtered_ids)):
      if colab_path is not None:
        ...
        if use_vision:
          ...
          v_df = None
        if use_audio:
            a_path = os.path.join(colab_path if colab_path else path_config.DATA_DIR, 'Audio Summary', f"{id}_audio_summary.csv")
      ...
      try:
        ...
        # Vision
        if use_vision and len(vision_seq_list) > 0:
          vision_dim = vision_seq_list[0].shape[-1]
          ...
          vision_dim = None
          x_vision = None
          data_vision_lengths = None

        # Audio
        if use_audio and len(audio_seq_list) > 0:
            audio_dim = audio_seq_list[0].shape[-1]
    ...
    if len(graphs) > 0:
    # x_vision이 있고, 그 안에 실제 데이터가 있을 때만 shape 참조
      v_dim = graphs[0].x_vision.shape[-1] if (hasattr(graphs[0], 'x_vision') and graphs[0].x_vision is not None) else 0
      a_dim = graphs[0].x_audio.shape[-1] if (hasattr(graphs[0], 'x_audio') and graphs[0].x_audio is not None) else 0
    else:
      v_dim, a_dim = 0, 0
