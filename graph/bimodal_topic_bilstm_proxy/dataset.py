...
from .._bimodal_model_bilstm.GAT import GATClassifier
...
from ..preprocessing import process_transcription, process_vision
  ...
  try:
    ...
    for graph_idx, id in tqdm(enumerate(filtered_ids), desc="Dataframe -> Graph", total=len(filtered_ids)):
      ...
      try:
        ...
        text_indices = [i for i, nt in enumerate(node_types) if nt not in ['vision']]
    ...
    if explanation:
      return graphs, (text_dim, v_dim), topic_node_id_dict
    else:
