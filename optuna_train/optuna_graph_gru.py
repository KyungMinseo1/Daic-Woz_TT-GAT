...
from graph_GRU._multimodal_model_gru.GAT import GATClassifier as GRUGAT, GATJKClassifier as GRUV2GAT
...
MODEL = {
  'multimodal_gru':GRUGAT,
  'multimodal_proxy':NoGRUGAT,
  'multimodal_topic_gru':GRUGAT,
  'multimodal_topic_gru_proxy':GRUGAT,
  'multimodal_topic_proxy':NoGRUGAT,
  'bimodal_topic_gru_proxy':BiGAT
}

V2_MODEL = {
  'multimodal_gru':GRUV2GAT,
  'multimodal_proxy':NoGRUV2GAT,
  'multimodal_topic_gru':GRUV2GAT,
  'multimodal_topic_gru_proxy':GRUV2GAT,
  'multimodal_topic_proxy':NoGRUV2GAT,
  'bimodal_topic_gru_proxy':BiV2GAT
}
...
def gru_objective(
    trial, config, mode, version, use_summary,
  ...
  gnl_list = [int(i) for i in config['model']['gru_num_layers_list']]
  ...
  gnl_min = min(gnl_list); gnl_max = max(gnl_list)
  ...
  gru_num_layers = trial.suggest_int("gru_num_layers", gnl_min, gnl_max)
...
def main():
  ...
  parser.add_argument('--use_summary', action='store_true',
                      help="Using Summary Node (if not activated, use topic nodes pooling).")
  ...
  try:
    if 'gru' in opt.mode:
      study.optimize(
          lambda trial: gru_objective(
            trial=trial, config=config, mode=opt.mode, version=opt.version, use_summary=opt.use_summary,
